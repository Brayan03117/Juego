...
def renderizar_escena(config, estado_juego, tablero):
    ...
    if config['personaje_id'] == 0:  # JesusL
        ...
        draw_dyson(config['dyson_posicion'],(0, 2, -2.2), emocion=config['dyson_emocion'])  # Ajusta la posición relativa si es necesario
    ...
    if config.get('mostrar_texto', True):
        ...
        dibujar_label_texto(f"Objeto Verde: +1 acierto", pos_x=10, pos_y=310, tam=15)
        dibujar_label_texto(f"No puede haber 2 numeros iguales en una fila o columna", pos_x=10, pos_y=280, tam=15)
        dibujar_label_texto(f"Objetivo: Completa el Sudoku", pos_x=10, pos_y=250, tam=15)
