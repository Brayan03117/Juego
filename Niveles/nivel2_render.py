...
def renderizar_escena(config, estado_juego, tablero):
    ...
    if config.get('mostrar_texto', True):
        dibujar_label_texto(f"Nivel 2 - Sudoku Presiona la Tecla T quitar/mostrar instrucciones", pos_x=10, pos_y=580, tam=20)
        ...
        dibujar_label_texto(f"Presiona 1-6 para insertar un número en la celda seleccionada", pos_x=10, pos_y=490, tam=15)
        ...
        dibujar_label_texto(f"Tiempo restante: {tiempo_str}", pos_x=10, pos_y=310, tam=15)
        dibujar_label_texto(f"Objeto azul: +10 segundos", pos_x=10, pos_y=280, tam=15)
        dibujar_label_texto(f"Objeto Naranja: -1 error", pos_x=10, pos_y=250, tam=15)
        dibujar_label_texto(f"Objeto Verde: +1 acierto", pos_x=10, pos_y=220, tam=15)
        dibujar_label_texto(f"No puede haber 2 numeros iguales en una fila o columna", pos_x=10, pos_y=190, tam=15)
        dibujar_label_texto(f"Objetivo: Completa el Sudoku antes de que termina el tiempo", pos_x=10, pos_y=160, tam=15)
