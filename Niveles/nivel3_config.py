...
from Esenarios import escenarioObjetos2 as es # Se podría cambiar a escenarioObjetos3 si se crea uno específico
...
def inicializar_nivel(personaje_id):
    ...
    pygame.display.set_caption("Sudoku - Nivel 3") # Cambiado a Nivel 3
    
    # Inicializar el módulo de sonido
    pygame.mixer.init()
    
    # Cargar los sonidos para cada escenario (pueden ser los mismos o diferentes para Nivel 3)
    sonidos_escenarios = {
    ...
    # Inicializar fondos (se podría usar un es_nivel3 si se crea)
    ...
    config = {
        ...
        'fondo_actual': 7, # Fondo inicial
        'jesus_posicion': 0,
        'torchic_posicion': 0,
        'dyson_emocion': "original",
        'dyson_posicion': 0,
        'luz_encendida': True, # La luz comienza encendida
        'cam_x': -4, 
        'cam_y': 0, 
        'cam_z': 15,
        'rot_x': 0, 
        ...
        'pistas_disponibles': 1 # Pistas para Nivel 3, se puede ajustar
    }
    ...
    glEnable(GL_LIGHTING) # La iluminación se maneja en el bucle principal para el parpadeo
