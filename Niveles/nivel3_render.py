...
def renderizar_escena(config, estado_juego, tablero):
    ...
    if config.get('mostrar_texto', True):
        ...
        dibujar_label_texto(f"Objeto Verde: +1 acierto", pos_x=10, pos_y=250, tam=15)
        dibujar_label_texto(f"No puede haber 2 numeros iguales en una fila o columna", pos_x=10, pos_y=230, tam=15)
        dibujar_label_texto(f"Objetivo: Completa el Sudoku antes de que termina el tiempo", pos_x=10, pos_y=200, tam=15)
